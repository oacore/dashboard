...
/**
 * Maps API row shape into {@link ArticleAdditionalData} so {@link CrDrawer} can render known fields (title, DOI, authors).
 */
export const mapFreshFindsRecordToArticle = (record: FreshFindsRecord): ArticleAdditionalData => {
    ...
    const idBase = doiRaw !== '' ? doiRaw : authorKey !== '' ? authorKey : 'fresh-finds-row';
    return {
