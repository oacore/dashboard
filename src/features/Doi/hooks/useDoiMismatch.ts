...
export const useDoiMismatch = (
    ...
    const key = (isLoaded && providerId)
        ? createSWRKey(`/internal/data-providers/${providerId}/doi/mismatch`, params)
        : null;
    ...
    const errorMessage = error instanceof Error ? error.message : error ? 'Failed to fetch DOI mismatch data' : null;
